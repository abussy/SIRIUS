...
C = ElementPsp(:C; psp=load_psp("./C_ghg.upf"; rcut=10.0))
atoms     = [C, C]                                                                                   
...
scfres = self_consistent_field(basis; ρ=ρ0, tol=1.0e-8, maxiter=40, mixing=SimpleMixing())#, eigensolver=diag_full)
...
scfres = self_consistent_field(basis_dftk; ρ=ρ0, tol=1e-8, maxiter=40, mixing=SimpleMixing())#, eigensolver=diag_full)
@show scfres.eigenvalues[1] .- scfres.eigenvalues[1][1]
#@show scfres.occupation[1]
@show scfres.energies
